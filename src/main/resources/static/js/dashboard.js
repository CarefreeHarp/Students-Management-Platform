(function () {
  ...
  function eventMarkup(task) {
    ...
    return `<a class="calendar-event" href="/proyectos/${encodeURIComponent(task.project.id)}" data-task="${data}" style="--event-color:${App.escapeHTML(task.project.color || '#5b5ce2')}">${App.escapeHTML(task.title)}<small>${App.escapeHTML(task.project.name)}</small></a>`;
  }
