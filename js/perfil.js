(function () {
  ...
  function renderProjects(projects) {
    ...
    if (!projects.length) {
      container.innerHTML = '<div class="profile-empty">Aún no participas en proyectos. <a href="./crear-proyecto.html">Crea el primero</a>.</div>';
      return;
    }

    container.innerHTML = projects.slice(0, 3).map((project) => `
      <a class="profile-project-card" href="./proyecto.html?id=${encodeURIComponent(project.id)}" style="--project-color: ${project.color}; --progress: ${project.progress}%">
  ...
  function setupEditForm(user) {
    ...
    form.addEventListener("submit", (event) => {
      ...
      window.setTimeout(() => window.location.assign("./perfil.html"), 450);
    });
  }
